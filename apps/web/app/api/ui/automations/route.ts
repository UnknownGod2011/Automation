...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (!name || !websiteUrl || !objective || !consentAcknowledged) return redirect(request, "/automations/new?notice=request-failed");
  ...
  try {
    ...
    if (error instanceof WebAuthError) return redirect(request, "/api/auth/sign-in?returnTo=/automations/new");
    if (error instanceof WebControlPlaneError && error.code === "NOT_CONFIGURED") return redirect(request, "/automations/new?notice=not-configured");
