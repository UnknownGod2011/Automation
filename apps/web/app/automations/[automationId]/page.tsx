...
import { publishRuntimeInputFields } from "../../../lib/publish-input-presentation";
...
export default async function AutomationDetailPage({
  ...
  const publishInputFields = workflowInspection ? publishRuntimeInputFields(workflowInspection) : null;
  const publishReady = publishWorkflowVersion !== null && publishInputFields !== null;
  ...
  return <>
    ...
    <section className="grid two">
      ...
      <div className="card stack"><h2>Approve and publish</h2><p>Publishing remains gated by the control plane: only the latest successfully tested immutable workflow version can be activated.</p>
        {runHistoryUnavailable ? <p className="muted">Publishing is temporarily paused because successful Fresh Test provenance cannot be verified while run history is unavailable.</p> : publishReady ? <form action={`/api/ui/automations/${encodeURIComponent(automationId)}/publish`} method="post"><label>Recurrence<select name="kind" defaultValue="DAILY"><option value="HOURLY">Hourly</option><option value="DAILY">Daily</option><option value="WEEKLY">Weekly</option><option value="CRON">Custom cron</option></select></label><label>Schedule expression<input name="expression" defaultValue="09:00" required /></label><label>Timezone<input name="timezone" defaultValue="Asia/Kolkata" required /></label>{publishInputFields.length > 0 ? <div className="stack"><p><strong>Reusable scheduled values</strong></p><p className="muted">Capture hides typed and selected values by design. Provide one reusable value for each semantic workflow step below. Internal workflow-variable names remain server-side. Do not enter passwords, OTPs, API keys, tokens, or other secrets; target-site authentication belongs in the Browser Profile.</p>{publishInputFields.map((field) => <label key={field.name}>{field.label.replace(" value", " reusable value")}<input name={field.name} type="text" maxLength={4_096} autoComplete="off" aria-label={`${field.label} reusable scheduled value`} required /><span className="muted">{field.guidance}</span></label>)}<label><input type="checkbox" name="scheduledInputsAreNonSecret" value="yes" required /> I confirm every value above is non-secret and safe to persist for future scheduled runs.</label></div> : <p className="muted">This workflow has no unresolved captured values, so no reusable scheduled inputs are required.</p>}<button className="button" type="submit">Approve and publish</button></form> : publishWorkflowVersion !== null && publishInputFields === null ? <p className="muted">Publishing is paused because the compiled workflow input requirements are invalid. Recompile the trusted capture rather than guessing at workflow variables.</p> : <p className="muted">Complete a successful fresh test before publishing. The tested workflow version is resolved from trusted run state rather than entered by the user.</p>}
