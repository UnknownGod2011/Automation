  ...
  try {
    ...
    if (error instanceof WebControlPlaneError && error.code === "NOT_CONFIGURED") {
      return (
        <section className="card">
          ...
          <h1>Connect the control plane to inspect this automation.</h1>
          <p>The browser UI does not fabricate automation data when its authenticated API boundary is unavailable.</p>
    ...
    return (
      <section className="card">
        ...
        <p>The control-plane request failed. Sensitive upstream error text is intentionally hidden.</p>
        <Link className="button" href="/">Back to dashboard</Link>
      </section>
    );
  }
  ...
  return (
    <>
      ...
      <section className="hero">
        <div>
          <div className="eyebrow">{automationPhase(automation)}</div>
          <h1>{automation.name}</h1>
          <p>{automation.objective}</p>
          <p className="muted">{automation.websiteUrl}</p>
        </div>
        <div className="card subtle stack">
          <div className="row"><span>Status</span><span className={automation.needsAttention ? "badge warning" : "badge"}>{automation.status}</span></div>
          <div className="row"><span>Workflow version</span><strong>{automation.publishedWorkflowVersion ?? "Not published"}</strong></div>
          <div className="row"><span>Schedule</span><span className="muted">{formatSchedule(automation)}</span></div>
        </div>
      </section>
      ...
      <section className="grid two">
        <div className="card stack">
          ...
          <div className="step"><div className="step-number">2</div><div><h3>Compile trace</h3><p>The trusted capture plane persists the trace. Until capture-state metadata is surfaced in the summary API, enter its server-issued trace ID here.</p><form action={`/api/ui/automations/${encodeURIComponent(automationId)}/compile`} method="post"><label>Trace ID<input name="traceId" required /></label><label>Workflow ID<input name="workflowId" defaultValue={`workflow-${automationId}`} required /></label><button className="button secondary" type="submit">Compile workflow</button></form></div></div>
        ...
        <div className="card stack">
          ...
          <p>Publishing remains gated by the control plane: only the latest successfully tested immutable workflow version can be activated.</p>
          <form action={`/api/ui/automations/${encodeURIComponent(automationId)}/publish`} method="post">
            <label>Workflow version<input name="workflowVersion" type="number" min="1" defaultValue={automation.publishedWorkflowVersion ?? 1} required /></label>
            <label>Recurrence<select name="kind" defaultValue="DAILY"><option value="HOURLY">Hourly</option><option value="DAILY">Daily</option><option value="WEEKLY">Weekly</option><option value="CRON">Custom cron</option></select></label>
            <label>Schedule expression<input name="expression" defaultValue="09:00" required /></label>
            <label>Timezone<input name="timezone" defaultValue="Asia/Kolkata" required /></label>
            <button className="button" type="submit">Approve and publish</button>
          </form>
          ...
          <div className="list">
            {runs.map((run) => (
              <div className="list-item" key={run.runId}>
                <div><h3>{run.runId}</h3><div className="muted">Scheduled {run.scheduledAt}</div></div>
                <div><span className={`badge ${runTone(run.status)}`}>{run.status}</span></div>
                <div className="muted">{run.failureCode ? `Failure: ${run.failureCode}` : run.currentNodeId ? `Node: ${run.currentNodeId}` : "—"}</div>
              </div>
            ))}
          </div>
        )}
