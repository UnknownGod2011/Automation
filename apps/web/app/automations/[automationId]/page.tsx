...
export default async function AutomationDetailPage({
  ...
  const freshTestReady = automation.status === "READY_TO_TEST" || automation.status === "READY_TO_PUBLISH";
  ...
  return (
    <>
      ...
      <section className="card stack" style={{ marginTop: 18 }}>
        ...
        {runs.length === 0 ? <p className="muted">No runs yet.</p> : <div className="list">{runs.map((run) => <div className="list-item" key={run.runId}><div><h3><Link href={`/automations/${encodeURIComponent(automationId)}/runs/${encodeURIComponent(run.runId)}`}>{run.runId}</Link></h3><div className="muted">{runKindLabel(run)} · {run.scheduledAt}</div></div><div><span className={`badge ${runTone(run.status)}`}>{run.status}</span></div><div className="muted">{run.failureCode ? `Failure: ${run.failureCode}` : run.currentNodeId ? `Node: ${run.currentNodeId}` : "—"}</div></div>)}</div>}
      </section>
    </>
  );
}
