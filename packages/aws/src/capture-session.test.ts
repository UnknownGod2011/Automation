...
describe("AgentCoreCaptureSessionStarter", () => {
  it("starts an isolated profiled AgentCore session and returns a time-bounded Live View URL", async () => {
    const { value, api, signer } = starter({
      sessionTimeoutSeconds: 3_600,
      liveViewTtlSeconds: 900,
    ...
    expect(result).toEqual({
      ...
      liveViewUrl: "https://bedrock-agentcore.us-west-2.amazonaws.com/live-view?signed=1",
    ...
    expect(signer.calls).toEqual([
      { browserIdentifier: "aws.browser.v1", sessionId: "session123", expiresInSeconds: 900 },
    ]);
  });
  ...
  it("rejects cross-tenant automation objects before creating browser compute", async () => {
    ...
    await expect(
      value.start(scope, { ...automation, tenantId: "tenant-other" }),
    ).rejects.toThrow(/ownership scope/);
