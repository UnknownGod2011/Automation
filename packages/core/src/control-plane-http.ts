...
export interface AuthenticatedControlPlaneContext {
  /** Trusted identity resolved by Cognito/auth middleware. Never parse ownership from request JSON. */
...
function stringField(body: Record<string, unknown>, name: string): string {
  ...
  if (typeof value !== "string") throw new ControlPlaneError("BAD_REQUEST", `${name} must be a string`);
  return value;
}
...
function booleanField(body: Record<string, unknown>, name: string): boolean | undefined {
  ...
  if (typeof value !== "boolean") throw new ControlPlaneError("BAD_REQUEST", `${name} must be a boolean`);
  return value;
}
...
function integerField(body: Record<string, unknown>, name: string): number {
  ...
  if (!Number.isInteger(value) || (value as number) < 1) {
    throw new ControlPlaneError("BAD_REQUEST", `${name} must be a positive integer`);
  }
...
export class AutomationControlPlaneHttpHandler {
  ...
  async handle(
    ...
    try {
      ...
      if (request.method === "GET" && parts.length === 2) {
        return { status: 200, body: await this.service.dashboard(context.scope) };
      }

      if (request.method === "POST" && parts.length === 2) {
        const body = jsonObject(request.body);
        const notifyOnSuccess = booleanField(body, "notifyOnSuccess");
        const notifyOnFailure = booleanField(body, "notifyOnFailure");
        const command: CreateAutomationCommand = {
          automationId: stringField(body, "automationId"),
          name: stringField(body, "name"),
          websiteUrl: stringField(body, "websiteUrl"),
          objective: stringField(body, "objective"),
          consentAcknowledged: booleanField(body, "consentAcknowledged") ?? false,
      ...
      if (request.method === "GET" && parts.length === 3) {
        return { status: 200, body: await this.service.getAutomation(context.scope, automationId) };
      }
      ...
      if (request.method === "POST" && parts[3] === "capture-trace" && parts.length === 4) {
        const body = jsonObject(request.body);
        const trace = body.trace as CaptureTrace | undefined;
        if (!trace) throw new ControlPlaneError("BAD_REQUEST", "trace is required");
        return { status: 202, body: await this.service.ingestCapture(context.scope, trace) };
      }

      if (request.method === "POST" && parts[3] === "compile" && parts.length === 4) {
        const body = jsonObject(request.body);
        const command: CompileAutomationCommand = {
          traceId: stringField(body, "traceId"),
          workflowId: stringField(body, "workflowId"),
        };
        return { status: 200, body: await this.service.compileAutomation(context.scope, automationId, command) };
      }

      if (request.method === "POST" && parts[3] === "test" && parts.length === 4) {
        const body = jsonObject(request.body);
        const runtimeVariables = body.runtimeVariables;
        if (
          runtimeVariables !== undefined &&
          (!runtimeVariables || typeof runtimeVariables !== "object" || Array.isArray(runtimeVariables))
        ) {
          throw new ControlPlaneError("BAD_REQUEST", "runtimeVariables must be a JSON object");
        }
        const command: TestAutomationCommand = {
          runId: stringField(body, "runId"),
          ...(runtimeVariables
            ? { runtimeVariables: runtimeVariables as Readonly<Record<string, unknown>> }
            : {}),
        };
      ...
      if (request.method === "GET" && parts[3] === "runs" && parts.length === 4) {
        return { status: 200, body: { runs: await this.service.history(context.scope, automationId) } };
      }
