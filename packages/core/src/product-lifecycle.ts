...
export interface PersistCaptureRequest {
  scope: OwnershipScope;
  trace: CaptureTrace;
}

export interface CompileAutomationRequest {
  scope: OwnershipScope;
  automationId: string;
  traceId: string;
  workflowId: string;
...
export interface FreshTestRunRequest {
  scope: OwnershipScope;
  automationId: string;
  runId: string;
...
export interface DispatchOccurrenceRequest {
  scope: OwnershipScope;
  automationId: string;
  scheduledAt: string;
  runId: string;
...
export class AutomationProductLifecycleService {
  ...
  async createDraft(request: CreateAutomationDraftRequest): Promise<AutomationRecord> {
    if (!request.consentAcknowledged) {
      throw new Error("explicit authorization/consent acknowledgement is required");
    }
  ...
  async persistCapture(request: PersistCaptureRequest): Promise<CaptureTrace> {
    ...
    if (!automation.browserProfileRef || automation.browserProfileRef !== request.trace.browserProfileRef) {
      throw new Error("capture trace browser profile does not match the automation");
    }
    if (!(await this.dependencies.profiles.exists(request.scope, automation.browserProfileRef))) {
      throw new Error("capture trace references an unavailable browser profile");
    }

    await this.dependencies.captures.putImmutable(request.trace);
    await this.dependencies.automations.put({
      ...automation,
      status: "COMPILING",
  ...
  async compile(request: CompileAutomationRequest): Promise<WorkflowGraph> {
    ...
    const trace = await this.dependencies.captures.get(
      request.scope,
    ...
    const graph = compileCaptureTrace({
      trace,
      workflowId: nonEmpty(request.workflowId, "workflowId"),
      version: nextVersion,
  ...
  async runFreshTest(request: FreshTestRunRequest): Promise<FreshTestRunResult> {
    ...
    if (!automation.browserProfileRef || !(await this.dependencies.profiles.exists(request.scope, automation.browserProfileRef))) {
      throw new Error("automation browser profile is unavailable");
    }
    ...
    if (!created.created) {
      return {
        kind: "DUPLICATE",
        run: created.run,
        checkpoint: await this.dependencies.checkpoints.get(request.scope, created.run.runId),
      };
    }
  ...
  async publish(request: PublishAutomationRequest): Promise<AutomationRecord> {
    ...
    const automation = await this.requireAutomation(request.scope, request.automationId);
    if (automation.status !== "READY_TO_PUBLISH") {
      throw new Error("automation must have a successful fresh test before publish");
    }
    ...
    if (latest.version !== graph.version) {
      throw new Error("only the latest successfully tested workflow version may be published");
    }
  ...
  private async seedFreshCheckpoint(
    scope: OwnershipScope,
    run: RunRecord,
    graph: WorkflowGraph,
    runtimeVariables?: Readonly<Record<string, unknown>>,
    ...
    const variables = {
      ...
      ...(copyRuntimeVariables(runtimeVariables) ?? {}),
    };
  ...
  private assertTraceOwnership(
    ...
    if (
      trace.tenantId !== scope.tenantId ||
      trace.userId !== scope.userId ||
      trace.automationId !== automation.automationId
    ) {
      throw new Error("capture trace ownership does not match automation scope");
    }
    if (normalizeWebsiteUrl(trace.websiteUrl) !== normalizeWebsiteUrl(automation.websiteUrl)) {
      throw new Error("capture trace website does not match automation");
    }
    if (trace.objective.trim() !== automation.prompt.trim()) {
      throw new Error("capture trace objective does not match automation");
    }
  }
}
