...
class RecordingBrowser implements BrowserExecutor {
  ...
  async executeDeterministic(
    ...
    return {
      effectObserved: true,
      evidenceRefs: [`memory-evidence://${runId}/${node.id}`],
      outputs: {},
      stateFingerprint: `${node.id}:ok`,
    };
  }
...
class AlwaysVerified implements VerificationEngine {
  async verify(context: VerificationContext): Promise<VerificationResult> {
    return {
      verified: true,
      evidenceRefs: [`memory-verification://${context.runId}/${context.node.id}`],
      detail: "fixture effect verified",
    };
  }
}
...
class UnexpectedReasoner implements ReasoningProvider {
  async decide(_request: ReasoningRequest): Promise<ReasoningDecision> {
    throw new Error("reasoner should not be called for deterministic fixture");
  }
}
    ...
    events: [
      {
        ...
        input: { kind: "RUNTIME_VARIABLE", variableName: "auth.password", sensitive: true },
        artifactRefs: [],
      },
      {
        eventId: "open-report",
        ...
        target: { testId: "open-report", role: "button", accessibleName: "Open report" },
        expectedEffect: {
          description: "Report editor is visible",
          mode: "TEXT",
      ...
      {
        eventId: "report-title",
        sequence: 3,
        kind: "INPUT",
        purpose: "WORKFLOW",
        occurredAt: "2026-08-19T12:00:05.000Z",
        page: { url: "https://example.test/app/report" },
        target: { testId: "report-title", role: "textbox", accessibleName: "Report title" },
        input: { kind: "PUBLIC_LITERAL", value: "Monthly report" },
        expectedEffect: {
          description: "Report title field contains the captured title",
          mode: "DOM",
          expected: "Monthly report",
          timeoutMs: 5_000,
        },
      ...
      },
      {
        ...
        kind: "INPUT",
        purpose: "WORKFLOW",
        occurredAt: "2026-08-19T12:00:07.000Z",
        page: { url: "https://example.test/app/report" },
        target: { testId: "recipient", role: "textbox", accessibleName: "Recipient" },
        input: { kind: "RUNTIME_VARIABLE", variableName: "report.recipient", sensitive: false },
        expectedEffect: {
          description: "Recipient field is populated",
          mode: "DOM",
          expected: "recipient-present",
          timeoutMs: 5_000,
        },
        artifactRefs: [],
      },
      {
        ...
        kind: "SUBMIT",
        purpose: "WORKFLOW",
        occurredAt: "2026-08-19T12:00:09.000Z",
        page: { url: "https://example.test/app/report" },
        target: { testId: "send-report", role: "button", accessibleName: "Send report" },
        expectedEffect: {
...
function harness() {
  ...
  const service = new AutomationProductLifecycleService({
    ...
    locks,
    browser,
    verifier: new AlwaysVerified(),
    reasoner: new UnexpectedReasoner(),
...
async function createCompile(h: ReturnType<typeof harness>) {
  ...
  const graph = await h.service.compile({
    ...
    workflowId: "workflow-automation-1",
  });
  return { draft, trace, graph };
}

describe("AutomationProductLifecycleService", () => {
  it("proves create -> capture -> compile -> fresh test -> publish -> scheduled execution -> history", async () => {
    const h = harness();
    ...
    const testResult = await h.service.runFreshTest({
      scope: owner,
      automationId: "automation-1",
      runId: "test-run-1",
      runtimeVariables: { "report.recipient": "finance-team" },
    });
    ...
    const typedValues = h.browser.calls
      .filter((call) => call.node.kind === "TYPE")
      .map((call) => call.inputs.value);
    expect(typedValues).toEqual(["Monthly report", "finance-team"]);

    const afterTest = await h.automations.get(owner, "automation-1");
    expect(afterTest?.status).toBe("READY_TO_PUBLISH");
    ...
    expect(published.publishedWorkflowVersion).toBe(graph.version);
    ...
    const scheduled = await h.service.dispatchOccurrence({
      scope: owner,
      automationId: "automation-1",
    ...
    const scheduledTypedValues = h.browser.calls
      .filter((call) => call.runId === "scheduled-run-1" && call.node.kind === "TYPE")
      .map((call) => call.inputs.value);
    expect(scheduledTypedValues).toEqual(["Monthly report", "ops-team"]);

    const callsAfterScheduledRun = h.browser.calls.length;
    const duplicate = await h.service.dispatchOccurrence({
      scope: owner,
      automationId: "automation-1",
      scheduledAt: "2026-08-20T03:30:00.000Z",
      runId: "scheduled-run-duplicate-delivery",
      runtimeVariables: { "report.recipient": "must-not-execute" },
    });
    ...
    expect(history.map((run: RunRecord) => run.runId)).toContain("test-run-1");
    expect(history.map((run: RunRecord) => run.runId)).toContain("scheduled-run-1");
  });
  ...
  it("keeps runtime values out of compiled workflow persistence while seeding them into fresh checkpoints", async () => {
    ...
    await h.service.runFreshTest({
      scope: owner,
      automationId: "automation-1",
      runId: "test-run-runtime",
      runtimeVariables: { "report.recipient": "runtime-team" },
    });
    ...
    expect(checkpoint?.variables["report.recipient"]).toBe("runtime-team");
  });
  ...
  it("fails closed on missing consent, cross-tenant access, capture replacement, and invalid publish ordering", async () => {
    ...
    ).rejects.toThrow("successful fresh test");

    await expect(
      h.captures.get(otherOwner, "automation-1", trace.traceId),
    ).resolves.toBeNull();
  });
  ...
  it("rejects invalid schedule timezones before activating an automation", async () => {
    ...
    await h.service.runFreshTest({
      scope: owner,
      automationId: "automation-1",
      runId: "test-before-invalid-schedule",
      runtimeVariables: { "report.recipient": "finance-team" },
    });
    ...
    await expect(
      h.service.publish({
        ...
        automationId: "automation-1",
        workflowVersion: graph.version,
        schedule: { kind: "DAILY", expression: "0 9 * * *", timezone: "Mars/Olympus" },
      }),
